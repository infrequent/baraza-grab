...
LUA_SHA1 = get_hash(os.path.join(CWD, 'baraza.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            "--warc-header", "baraza-dld-script-version: " + VERSION,
            "--warc-header", ItemInterpolation("baraza-user: %(item_name)s"),
        ]
...
project = Project(
    title="baraza",
    project_html="""
        <img class="project-logo" alt="Project logo" src="http://archiveteam.org/images/4/40/Google_Logo.png" height="50px" title=""/>
        <h2>www.google.com/baraza/ <span class="links"><a href="http://www.google.com/baraza/">Website</a> &middot; <a href="http://tracker.archiveteam.org/baraza/">Leaderboard</a></span></h2>
        <p>Saving all question and answers from Baraza.</p>
    """,
    utc_deadline=datetime.datetime(2015, 4, 30, 23, 59, 0)
)
...
pipeline = Pipeline(
    ...
    GetItemFromTracker("http://%s/%s" % (TRACKER_HOST, TRACKER_ID), downloader,
        VERSION),
    PrepareDirectories(warc_prefix="baraza"),
    WgetDownload(
        WgetArgs(),
        max_tries=2,
